...
RunNumber = 10 #How many times the ensemble will run each set of values (since random ellipse placement and orientation is used, this number should be more than 1. 10 is recommended)
...
for pn in range(EnsembleNumber):
    ...
    for run in range(RunNumber):
        ...
        w = Wlist[pn] #Equation of state parameter of dark energy (between -1/3 and -5/3), measured to be ~-1
        c = 2.9979*10**5 #km/s speed of light
        Hubble = Hubblelist[pn] #(Hubble constant)
        DE_Ratio = AnisotropyRatiolist[pn] #1 # must be greater than 0 but less than or equal to 1 (0.9 equates to 0.9*H0 in the y-direction)
        xaxisminutes = 4.4 # How big the Deep Field Image will be in the x-direction (arcminutes) (4.4 for JWST, 3.1 for HST)
        yaxisminutes = 2.2 # How big the Deep Field Image will be in the y-direction (arcminutes) (2.2 for JWST, 3.1 for HST)
        N_Unseen = Unseenlist[pn] # How many times more galaxies exist than are seen in UDF: anywhere from 2 to 10
        R_Increase = Halflightlist[pn] # % of the radius seen due to higher sensitivity of telescope: 100% means anything beyond the halflight radius is invisible, 160 means the visible radius is 1.60 times larger than the effective radius in Allen et al.'s equation
        ...
        ZMAX = Zmaxlist[pn]  #The maximum redshift reached by the telescope (11 for UDF, 15 for JWST)
        
        #Initial values for first 2 images. These do not impact the deep field simulation.
        Length = 0.025 # proper length (Mpc) for first two graphs
        ZValue = 20 #Used to calculate apparent magnitude in first two graphs
        """
        -----------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        print('Apparent Magnitude of Test Length= '+str(round(theta,4)) +' arcminutes')
        ...
        Galaxies are generated using randomly generated ellipses. The major axis of each ellipse 
        is equal to the apparent angular size at the specified z values. The minor axis is a 
        random value between 1% the apparent angular size and 100% to simulate a random 
        face-orientation of each galaxy since galaxies are seen anywhere from edge-on to face-on. 
        The 1% lower boundary comes from the Milky Way's own proportions. Each galaxy is also 
        ...
        plt.savefig('Ensemble Number '+str(pn)+'.png',dpi=dpi)   
        ...
        #Close all figures so the number of open figures does not continue to increase
        plt.close()
        plt.close()
        plt.close()
        plt.close()
...
#Create an audio notification when the ensemble simulation has completed
import winsound
