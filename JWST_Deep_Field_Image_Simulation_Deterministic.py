...
DE_Ratio = 0.5 # 1 means isotropic expansion, 0.9 equates to 90% of expansion (0.9*H_0) in the y-direction, etc.
...
def YearsToSeconds(x):
    x = x*3.154*10**7
    return x

def AgeUniverse(z): #Integration to find the universe's age
    return (MpcsToKm(1)/YearsToSeconds(Hubble))/((((1-omegatotal)*(1+z)**2 + omegamass*(1+z)**3 + omegalambda*(1+z)**m)**0.5)*(1+z))

#Function of SAHNI, V., & STAROBINSKY, A. (2000)'s equations.
def ApparentAngularDiameter(input,i):
    if k == 0:
        ans, err = quad(InverseDensities, 0, i)
...
ans, err = quad(GalaxyNumberDensity, 0, ZMAXHUDF)
